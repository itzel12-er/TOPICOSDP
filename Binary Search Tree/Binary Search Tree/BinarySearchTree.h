...
public:
    ...
    void DeleteTreePostOrder()
    {
        ...
        while (!stack.empty())
        {
            ...
            if (current != lastVisited && current != nullptr)
            {
                stack.pop();
    ...
    // Nueva función PostOrder iterativa
    void PostOrderIterative()
    {
        ...
        // Recorremos el árbol en post-order
        while (current != nullptr || !stack.empty())
        {
            // Vamos lo más a la izquierda posible, apilando nodos
            ...
            // Miramos el nodo en la cima de la pila
            current = stack.top();

            // Si el hijo derecho no existe o ya fue visitado, procesamos el nodo
            if (current->rightChild == nullptr || current->rightChild == lastVisited)
            {
                ...
                stack.pop();
                lastVisited = current;
                current = nullptr; // Forzamos a tomar el siguiente nodo de la pila
            }
            else
            {
                // Si no, exploramos el subárbol derecho
